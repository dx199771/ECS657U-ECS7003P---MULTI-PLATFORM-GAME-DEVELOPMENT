...
public class PlayerAnimation : MonoBehaviour
{
    ...
    public void SetDirection(Vector2 _direction) {
        ...
        if (_direction.magnitude < 0.01) //if player speed less than 0.1
        {
            directionArray = staticDirections; //set current move to static
            //lastDirection = 2;

        }
        else {
            directionArray = runDirections; //otherwise set current move to run
